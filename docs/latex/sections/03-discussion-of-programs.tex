	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item \textbf{ZO} is the axial distance to the end of a buried injector in ft. (\cite{ref1_annual_report_1}).
			\item \textbf{GO} is the inlet mass flow rate in $lb/ft^{2}-sec$. It must be greater than zero.
			\item \textbf{FC} is the rate of feed of hydrazine from buried injectors (\cite{ref1_annual_report_1}) into the system in $lb/ft^{3}$-sec.
			\item \textbf{ALPHA3} is the preexponential factor in the rate equation for the thermal decomposition of hydrazine (See \cite{ref1_annual_report_1}).
		...
		\begin{itemize}
			...
			\item \textbf{ALPHAL} is the preexponential factor in the rate equation for the catalytic decomposition of hydrazine (See \cite{ref1_annual_report_1}).
			For the Shell 405 catalyst it equals $10^{10} sec^{-1}$.
			\item \textbf{ALPHA2} is the preexponential factor in the rate equation for the catalytic decomposition of ammonia (See \cite{ref1_annual_report_1}).
...
\begin{enumerate}
	...
	\item Cards four contain the values of \textbf{F(I)}, (the rates of feed of hydrazine from buried injectors (\cite{ref1_annual_report_1}) into the system in $lb/ft^{3}-sec$). One value of F for each radial station (total number of radial stations = NRINGS) should be input. Ten numbers are allowed to a card. For the suggested NRINGS of 10, there would be one card with ten values of F.
